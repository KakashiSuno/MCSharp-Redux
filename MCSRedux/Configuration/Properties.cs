...
namespace MCSRedux.Configuration
{
    public static class Properties
    {
		...
		public static void Load(string path)
		{
			...
			while(!r.EndOfStream)
			{
				...
				case "allow-flight":
					if(!Boolean.TryParse(val, out allowflight))
						MCSR.log.Write(LogType.Warning, "config option 'allow-flight' has invalid value. Using default");
					break;
				case "difficulty":
					if(!Byte.TryParse(val, out difficulty))
						MCSR.log.Write(LogType.Warning, "config option 'difficulty' has invalid value. Using default");
					break;
				case "gamemode":
					if(!Byte.TryParse(val, out gamemode))
						MCSR.log.Write(LogType.Warning, "config option 'gamemode' has invalid value. Using default");
					break;
				case "max-players":
					if(!Int16.TryParse(val, out maxplayers))
						MCSR.log.Write(LogType.Warning, "config option 'max-players' has invalid value. Using default");
					break;
				case "motd":
					motd = val;
					break;
				case "pvp":
					if(!Boolean.TryParse(val, out pvp))
						MCSR.log.Write(LogType.Warning, "config option 'pvp' has invalid value. Using default");
					break;
				case "view-distance":
					if(!Byte.TryParse(val, out viewdistance))
						MCSR.log.Write(LogType.Warning, "config option 'view-distance' has invalid value. Using default");
					break;
				case "white-list":
					if(!Boolean.TryParse(val, out whitelist))
						MCSR.log.Write(LogType.Warning, "config option 'white-list' has invalid value. Using default");
					break;
				case "enable-query":
					if(!Boolean.TryParse(val, out enablequery))
						MCSR.log.Write(LogType.Warning, "config option 'enable-query' has invalid value. Using default");
					break;
				case "enable-rcon":
					if(!Boolean.TryParse(val, out enablercon))
						MCSR.log.Write(LogType.Warning, "config option 'enable-rcon' has invalid value. Using default");
					break;
				case "online-mode":
					if(!Boolean.TryParse(val, out onlinemode))
						MCSR.log.Write(LogType.Warning, "config option 'online-mode' has invalid value. Using default");
					break;
				case "server-ip":
					...
					if(System.Net.IPAddress.TryParse(val, out tmp))
						...
						MCSR.log.Write(LogType.Warning, "config option 'server-ip' has invalid value. Using default");
					break;
				case "server-port":
					if(!UInt16.TryParse(val, out serverport))
						MCSR.log.Write(LogType.Warning, "config option 'server-port' has invalid value. Using default");
					break;
				case "allow-nether":
					if(!Boolean.TryParse(val, out allownether))
						MCSR.log.Write(LogType.Warning, "config option 'allow-nether' has invalid value. Using default");
					break;
				case "level-name":
				...
				case "spawn-animals":
					if(!Boolean.TryParse(val, out spawnanimals))
						MCSR.log.Write(LogType.Warning, "config option 'spawn-animals' has invalid value. Using default");
					break;
				case "spawn-monsters":
					if(!Boolean.TryParse(val, out spawnmonsters))
						MCSR.log.Write(LogType.Warning, "config option 'spawn-monsters' has invalid value. Using default");
					break;
				default:
					MCSR.log.Write(LogType.Error, "Unknown config option: '" + key + "'");
					break;
				}
			}
			...
			if(maxplayers > 127)
				MCSR.log.Write(LogType.Warning, "'max-players' set to a value higher than 127. This will cause the ingame player list to not work!");
		}
		/// <summary>
