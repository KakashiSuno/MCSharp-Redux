...
namespace Minecraft_Server
{
    ...
    public class MapGenerator
    {
        ...
        {
            Server.Log("Attempting map gen");
            if (Inuse) { Server.Log("Generator in use"); return false; }
            ...
            try
            {
                ...
                Server.Log("Creating overlay");
                GeneratePerlinNoise(overlay, Lvl, "", rand);

                if (!type.Equals("ocean"))
                {
                    Server.Log("Planning trees");
                    GeneratePerlinNoise(overlay2, Lvl, "", rand);
                }

                Server.Log("Converting height map");
                Server.Log("And applying overlays");
                float RangeLow = 0.2f;
        ...
        void GenerateFault(float[] array,Level Lvl, string type, Random rand)
        {
            ...
            Server.Log("Irrerations = " + numIterations.ToString());
            for (k = 0; k < numIterations; k++)
            {
                //Server.Log("itteration " + k.ToString());
                ...
                for (i = 0; i < Lvl.height; i++)
                {
                    for (j = 0; j < Lvl.width; j++)
                    {
                        ...
                        //Server.Log("adding " + dispAux.ToString());
        ...
        {
            Server.Log("Applying average filtering");
